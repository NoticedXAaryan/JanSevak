...
async def test_handle_text_message_success(
    mock_update, mock_thinking, mock_process, mock_session_factory, mock_message
):
    # Setup mocks
    ...
    mock_process.assert_called_once_with(telegram_id=123456789, user_text="Hello JanSeva")
    mock_update.assert_called_once_with(
        mock_message, thinking_msg, "This is a response", reply_markup=None
    )
...
async def test_handle_text_message_unonboarded(
    mock_handle_start, mock_session_factory, mock_message
):
    # Setup mocks
    mock_session = AsyncMock()
    mock_session_factory.return_value.__aenter__.return_value = mock_session
