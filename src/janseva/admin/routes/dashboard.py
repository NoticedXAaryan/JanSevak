...
async def dashboard_home(
    ...
    users_count = await session.scalar(select(func.count()).select_from(User)) if admin_user["role"] == "super_admin" else 0
    
    # 2. Total Conversations (Super Admin only for now)
    conversations_count = await session.scalar(select(func.count()).select_from(Conversation)) if admin_user["role"] == "super_admin" else 0
    
    # 3. Active Escalations (Not yet implemented, just a placeholder)
    escalations_count = 0
    
    # 4. Pending Reports
    reports_query = select(func.count()).select_from(AnonymousReport).where(AnonymousReport.status == "pending")
    if admin_user["role"] != "super_admin":
        if admin_user.get("org_id"):
            reports_query = reports_query.where(AnonymousReport.organization_id == admin_user["org_id"])
        else:
