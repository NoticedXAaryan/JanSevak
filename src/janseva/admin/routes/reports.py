    ...
    if admin_user["role"] == "super_admin":
        ...
        if not org_id:
            return templates.TemplateResponse(
                request=request,
                name="reports.html",
                context={"request": request, "admin": admin_user, "reports": []},
            )

        stmt = (
            select(AnonymousReport)
            .where(AnonymousReport.organization_id == org_id)
...
async def update_report_status(
    ...
    stmt = update(AnonymousReport).where(AnonymousReport.id == uuid.UUID(report_id))
    ...
    if admin_user["role"] != "super_admin":
        ...
        if not org_id:
            return HTMLResponse(
                content="<span class='text-red-500'>Unauthorized</span>", status_code=403
            )
