...
down_revision: Union[str, None] = 'e3198948670c'
...
def upgrade() -> None:
    ...
    op.execute("CREATE INDEX IF NOT EXISTS idx_conversations_user_active ON conversations(user_id) WHERE status = 'active';")
    op.execute("CREATE INDEX IF NOT EXISTS idx_messages_conversation ON messages(conversation_id, created_at DESC);")
    op.execute("CREATE INDEX IF NOT EXISTS idx_reports_status ON anonymous_reports(status) WHERE status != 'resolved';")
